    ...
    <Card className="w-full max-w-md border-2 md:border-4 border-black shadow-neo bg-white">
      ...
      <form onSubmit={handleSubmit} className="mt-4 md:mt-6">
        <CardContent className="space-y-4 md:space-y-6">
          ...
          {message && (
            <div className="p-3 md:p-4 text-[10px] md:text-sm font-black uppercase bg-green-100 border-2 border-black shadow-neo-sm text-green-600">
              {message}
            </div>
          )}
        ...
        <div className="p-6 pt-0">
          <Button
            type="submit"
            disabled={loading}
            className="w-full border-2 border-black shadow-neo-sm hover:shadow-none transition-all h-10 md:h-12 font-black uppercase italic tracking-tighter bg-primary"
          >
            {loading ? "ĐANG XỬ LÝ..." : (token ? "CẬP NHẬT MẬT KHẨU" : "GỬI LIÊN KẾT")}
          </Button>
          <div className="mt-4 text-center">
            <a href="/login" className="text-[10px] md:text-xs font-black uppercase underline italic">
              Quay lại đăng nhập
            </a>
          </div>
        </div>
