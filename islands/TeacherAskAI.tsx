...
export default function TeacherAskAI({ user }: TeacherAskAIProps) {
  ...
  useEffect(() => {
    const storedMessages = localStorage.getItem(`ai_conversation_teacher_${user.id}`);
    if (storedMessages) {
      try {
  ...
  useEffect(() => {
    if (messages.length > 0) {
      localStorage.setItem(`ai_conversation_teacher_${user.id}`, JSON.stringify(messages));
    }
  }, [messages, user.id]);
  ...
  const clearConversation = () => {
    setMessages([]);
    localStorage.removeItem(`ai_conversation_teacher_${user.id}`);
  };
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs md:text-sm text-black font-bold uppercase tracking-tight opacity-70">
                AI trợ lý chuyên môn cho giáo viên
              </p>
            </div>
            <button
              ...
              <div className="flex items-center justify-center h-full">
                <div className="text-center">
                  ...
                  <p className="text-sm md:text-base font-black uppercase italic text-black opacity-70">
                    Xin chào thầy/cô! Tôi có thể hỗ trợ gì?
                  </p>
                  <p className="text-xs md:text-sm text-gray-600 mt-2">
                    Hãy hỏi tôi về phương pháp giảng dạy, tài liệu, hoặc bất kỳ vấn đề nào
                  </p>
                </div>
              </div>
            ) : (
