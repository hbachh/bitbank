...
export const handler = {
  async GET(req: Request, ctx: FreshContext) {
    ...
    if (!assignment) {
      return new Response("Practice assignment not found", { status: 404 });
    }
    ...
    // Fetch submissions for attempts and review
...
export default function PracticeSessionPage({ data }: PageProps) {
  const { 
    user, 
    assignment, 
    questions: assignmentQuestions, 
    ...
    <DashboardLayout user={user} pathname={pathname}>
      <div className="mb-6 flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black uppercase italic tracking-tighter text-black">
              {assignment.title}
            </h1>
            <p className="text-[10px] font-black uppercase opacity-70 text-black">
              {isReview ? "CHẾ ĐỘ XEM LẠI" : "ĐANG TỰ LUYỆN TẬP"} • LƯỢT: {attemptsCount} / {maxAttempts}
            </p>
          </div>
        </div>
        {isReview && attemptsCount < maxAttempts && (
          <a href={pathname}>
            <button className="p-2 px-4 border-4 border-black bg-primary hover:bg-primary/80 shadow-neo-sm font-black uppercase italic text-xs text-black">
              LÀM LẠI BÀI
            </button>
          </a>
        )}
